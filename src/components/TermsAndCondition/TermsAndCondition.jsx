...
const TermsAndCondition = () => {
    const [studentTermsAndCondition, setStudentTermsAndCondition] = useState(null);

    const handleChange = (e) => {
        const { files } = e.target;
        setStudentTermsAndCondition(files[0]);
    }

    return (
        <div>
            <h1 className='text-base font-medium m-1 md:ml-3 mt-4'>Terms & Condition</h1>
            ...
            <label className='text-xs m-1 md:ml-3 block'>
                <input
                    type='checkbox'
            ...
            <div className='flex flex-col md:flex-row'>
                <div className='md:basis-1/2 md:ml-10 lg:ml-16'>
                    ...
                    <p className='text-xs ml-1 mb-1'>(Supported Format: jpeg/jpg/png)</p>
                    {studentTermsAndCondition && (
                        <h1 className='ml-1 mb-1 md:mr-10 text-sm'>
                            File uploaded: {studentTermsAndCondition.name}
                        </h1>
                    )}
                </div>
